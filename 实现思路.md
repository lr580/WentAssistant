...
- 初始化 `ui.TbMain` 控制栏
- 重置 `ui.DbTable.h` 列名
- (可选) 在 `ui.DbTable` 的 `ui.TableBlender` 接口 `blender` 对数据库查询结果显示格式作出修改
- (可选) 在 `ui.DbTable` 的 `ui.InputFiller` 接口 `filler` 对自动补全的控件和规则作出修改
- (可选) 在 `ui.DbTable` 的 `ui.TableUpdater` 接口 `updater` 对数据库更新结果显示格式作出修改
- (可选) 在 `ui.TbGlobal` 的 `EvalCtrl` 接口 `pro_save, pro_undoall` 作出保存，撤销全部时的特殊动作修改
- 初始化指令管理器，执行 `ProcessCtrl.init();`
- (可选)记录日志文件
...
- `preference.txt` 偏好设置文本
  ...
  - `isEncryptDiary` 是否对保存的日志加密(1或**0**)
...
- `DateHelper` 日期运算简化类
  ...
  - `public static int getWeek(int y, int m, int d)` 返回星期 $[0,6]$
...
- `Supply` 辅助函数类
  ...
  - `public static int getMultiInput(double[] money, int[] date, int[] type, String[] comment, String s_money, String s_date, int s_type, String s_comment, String s_cmd)` 获得多选输入并返回输入数目
