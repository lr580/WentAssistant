...
- (废弃) 在 `ui.DbTable` 的 `ui.TableFetchConvertor` 接口 `convertor` 对自动补全的控件和规则作出修改
...
- `preference.txt` 偏好设置文本
  ...
  格式为若干行，每行 `key=value` ，各 key 如下：

  - `nowModule` 当前选择的模块名
  - `isRecordDiary` 是否保存日志(1或0)
  - `isEncryptDiary` 是否对保存的日志加密(1或0)
  - `isClearAfterAdd` 是否每次插入后清除输入框(1或0)
  - `isFillAfterSelect` 是否选择后填入输入框(1或0)
